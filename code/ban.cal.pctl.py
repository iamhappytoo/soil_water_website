...
#	pctl =  np.empty(len(aalld), dtype=float)
	...
	uudate = np.unique(ddate) ###!!!!so far we are in 2018 so we do not need to do so. But later we are not 2018, this will still work
...
# prepare data
	alld = np.loadtxt("/raid4/forecast/pnw_sw_monitor/soil_moisture_obs_update/03.CALC_PERCENT/sm.history/"+nam)
...
#	p = pathlib.Path("/raid4/forecast/pnw_sw_monitor/soil_moisture_obs_update/03.CALC_PERCENT/pctl.from.2018/")
	fout = open("/raid4/forecast/pnw_sw_monitor/soil_moisture_obs_update/03.CALC_PERCENT/pctl.from.2018/pctl."+nam, "w")
...
#	    for i in range(np.shape(datam)[0]):
#		indsm = np.mean(np.nonzero(sm_od==sm[i])[0])
#		pctl = (indsm+1-0.3)/(len(sm)+0.4)
#		fout.write(str(int(datam[i,0]))+' '+\
#	                   str(int(datam[i,1]))+' '+\
#        	           str(int(datam[i,2]))+' '+\
#                	   "{0:.4f}".format(pctl)+"\n")
