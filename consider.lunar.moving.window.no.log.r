library(lubridate)
library(timeDate)
##This is a script that consider lunar year, moving window, no log transform, and also consider to put timeseries into original array as update. 
...
for (i in 1:7305)
{
  numdate[i]=data[i,1]  ####!!!!!reversed store of original data!
}
for(i in 1:7305){
  for(j in 1:31){
    if(i+j-1==7305){
...
result=smobs[,16]
setwd("/home/zban/Downloads/02.15.smdatabase/wholets/vicwholets/movingwindow.no.log.with.update.result/")
